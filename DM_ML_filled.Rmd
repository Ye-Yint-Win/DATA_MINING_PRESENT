...
history <- modnn %>% 
  fit(
    x[-testid , ], y[-testid], epochs = 1000, batch_size = 32, validation_data = list(x[testid , ], y[testid])
    )
```
...
Please re-reroute the folder that your downloaded dataset is in.
This dataset is 60,000 images in the training data and 10,000 in the test data. The images are 28×28, and stored as a three-dimensional 
...
dim(test_images)
```
After array reshaping, we will put the images data into 8-bit greyscale values from 0 to 255 (256). 
```{r}
...
Model for multi-layer neural networks using relu twice, and output layer with activation function of "softmax".
The (1st) first layer goes from 28 × 28 = 784 input units to a hidden layer of 256 units, followed with dropout layer to perform regularization.
This will have 128 hidden units into the second (2nd) layer of Sigmoid model, followed with dropout layer.
The output layer for this model will have "soft-max" activation for 10-class classification.
...
The model will itereate 20 times, and it processes 128 samples at each time. 
0.2 of validation_split is so that 20% is used for validation, while the rest, 80%, is for training. 
Categorical Crossentropy is well-suited for multi-class classification. 
```{r}
modelnn %>% compile(
...
accuracy() that compares predicted and true class labels, and this computes the correctly predicted labels out of the total number of labels.
k_argmax(): convert the predictions into class labels.
...
Now, we'll model with a single layer to get a comparable model to the multi-layer model above.
...
```{r}
modelr %>% predict(test_images) %>% k_argmax() %>% accuracy(test_labels)
...
HomeWork
Here's a BRCA dataset that is prepared for regression with single-layer neural network.
```{r}
data(brca)
...
1. Fit a linear model to predict `x.radius_mean` and get MAE for the model
```{r}
...
3. Make a single-layer neural network with an activation layer of your choice, a dropout layer and output layer.
